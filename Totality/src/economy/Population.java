...
public class Population {
	 ...
	 * 
	 * @param households
	 *            Population in which we make the merge
	 * @param target
	 *            Target HH in the population
	 * @param source
	 *            Source HH in the population
	 */
	public void mergeHhs(int target, int source) {
		if (target <= households.size() && source <= households.size()
				&& target >= 0 && source >= 0 && source != target) {
			for (int i = households.get(source).members.size() - 1; i >= 0; i--) {
				households.get(target).members
						.add(households.get(source).members.get(i));
			}
		...
		if (possibleMatch.size() > 0) {
			...
			for (int i = 0; i < possibleMatch.size(); i++) {
				...
				while (j < possibleMatch.size()) {
					boolean notSameSex = possibleMatch.get(i).members.get(0).sex 
							!= possibleMatch.get(j).members.get(0).sex;
					double ageDiff = Math.abs(
					...
					if (notSameSex && (1.5 / (2 + ageDiff) > rand.nextDouble())) {
						...
						// Marry people
						possibleMatch.get(i).marry(possibleMatch.get(i).hoh,
								possibleMatch.get(i).members.get(1));
						// Merge incomes
		...
		for (int i = 0; i < households.size(); i++) {
			for (int j = 0; j < households.get(i).members.size(); j++) {
				if (households.get(i).members.get(j).age == age) {
					households.get(i).delMember(
							households.get(i).members.get(j));
					j--;
					Pers.nPers--;
				}
			}
